...
highp vec4 directional_light(highp vec3 point, highp vec3 normal, highp vec3 view_position, highp vec3 light_dir){
    ...
    highp mat3 matereal = mat3(Ca.xyz, Cd.xyz, Cs.xyz);
    highp mat3 light = mat3(Ga.xyz, Ld.xyz, Ls.xyz);
    highp vec3 factor = vec3(1, max(cosTheta, 0.), max(cosAlphaN, 0.));

    //cosTheta = 0;
    //cosAlphaN  = 0;
    return vec4(matrixCompMult(matereal, light) * factor, 1);
}
void main(void)
{
