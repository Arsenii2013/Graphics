...
highp vec4 point_light(highp vec3 point, highp vec3 normal, highp vec3 view_position, highp vec3 light_point){
    ...
    highp mat3 matereal = mat3(vec3(0., 0., 0.), Cd.rgb, Cs.rgb);
    highp mat3 light = mat3(vec3(0., 0., 0.), Ld.rgb, Ls.rgb);
    highp vec3 factor = vec3(0., light_I * max(cosTheta, 0.), light_I * max(cosAlphaN, 0.));

    return vec4(matrixCompMult(matereal, light) * factor, 0.);
}

highp vec4 ambient(){
    return vec4(Ca.r * Ga.r, 0, Ca.b * Ga.b, 1.);
}
